 ...
 * </ul>
 */
CQ.form.rte.plugins.FontColorPlugin = CQ.Ext.extend(CQ.form.rte.plugins.Plugin, {
    ...
    constructor: function(editorKernel) {
        CQ.form.rte.plugins.FontColorPlugin.superclass.constructor.call(this, editorKernel);
    },
    ...
    initializeUI: function(tbGenerator) {
        ...
        if (this.isFeatureEnabled("styles")) {
            this.stylesUI = new ui.TbFontColorSelector("fontcolor", this, null, this.getStyles());
            tbGenerator.addElement("fontcolor", plg.Plugin.SORT_STYLES, this.stylesUI, 10);
        }
    },
    ...
    updateState: function(selDef) {
        ...
        if (styleableObject) {
            if (!CQ.form.rte.Common.isTag(selDef.selectedDom,
                    CQ.form.rte.plugins.FontColorPlugin.STYLEABLE_OBJECTS)) {
        ...
        if (styleableObject) {
            ...
            for (var c = 0; c < checkCnt; c++) {
                ...
                var styleToProcessDiv = document.createElement('DIV');
            	styleToProcessDiv.style.color=styleToProcess;
                for (s = 0; s < styleCnt; s++) {
                	var stylesDefDiv = document.createElement('DIV');
                	stylesDefDiv.style.color=stylesDef[s].cssName;
                    if (stylesDefDiv.style.color==styleToProcessDiv.style.color) {
        ...
        if (actualStyles.length == 0) {
            ...
            indexToSelect = -1;
                ...
                for (var optIndex = 0; optIndex < options.length; optIndex++) {
                    ...
                    var optionToCheckDiv = document.createElement('DIV');
                    if (optionToCheck.value!='none'){
	                	styleToProcessDiv.style.color=optionToCheck.value;
	                	
	                    if (styleToProcessDiv.style.color == styleToSelect) {
...
// register plugin
CQ.form.rte.plugins.PluginRegistry.register("fontcolor", CQ.form.rte.plugins.FontColorPlugin);