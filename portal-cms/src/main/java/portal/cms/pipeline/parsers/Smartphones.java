...
public class Smartphones extends SlingAllMethodsServlet {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(Smartphones.class);
    ...
    private void generateClientLibsAndImageFromHTML(String html) throws IOException, ServletException {
        ...
        for (Element jsTag : jsLibrariesElements) {
            ...
            if ((jsPath.startsWith("/etc/") || jsPath.startsWith("/apps/") || jsPath.startsWith("/libs/")) && !new File(fullJsPath).exists()) {
                writeTextFile(fullJsPath, processResource(additionalPathToClientLibs + jsPath));
            }
        }
    ...
    private void checkCssJsFiles(String data) {
        // StringUtils.
    }
